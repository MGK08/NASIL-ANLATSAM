...
export function OdaEkrani() {
  ...
  useEffect(() => {
    ...
    if (inGame && iHaveSlot) router.push(`/oyun?kod=${code}`);
  ...
  async function pick(slotId: string) {
    ...
    if (!res.ok) setErr("İsim seçilemedi (" + res.reason + ")");
    setBusy(false);
  }
  async function start() {
    setBusy(true);
    setErr("");
    const res = await postAction({ type: "START_GAME", code, byUserId: getUserId() });
    if (!res.ok) setErr("Başlatılamadı (" + res.reason + ")");
    setBusy(false);
  }
